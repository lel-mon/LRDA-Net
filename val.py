...
def parse_args():
    ...
    parser.add_argument('--name', default='ich_LRDA_Net', help='model name')
...
def main():
    ...
    model.load_state_dict(torch.load('models/%s/model.pth' % args.name), strict=False)
    ...
    for c in range(config['num_classes']):
        os.makedirs(os.path.join('val_outputs', args.name, str(c)),
                    exist_ok=True)
    ...
    with torch.no_grad():
        for input, target, meta in tqdm(val_loader, total=len(val_loader)):
            ...
            for i in range(len(output)):
                for c in range(config['num_classes']):
                    cv2.imwrite(os.path.join('val_outputs', args.name, str(c), meta['img_id'][i] + '.png'),
