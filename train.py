...
def parse_args():
    ...
    parser.add_argument('--input_w', default=512, type=int, help='images width(default: 512)')
    parser.add_argument('--input_h', default=512, type=int, help='images height(default: 512)')
    ...
    # data
    parser.add_argument('--dataset', default='ISIC', help='dataset name')  ### isic, busi, aims
...
def main():
    ...
    # count param flops
    # m_input = torch.randn(1, 3, 512, 512)
    # flops, params = profile(model, (m_input,))
    ...
    # Data loading code
    img_ids = glob(os.path.join('inputs', config['dataset'], 'train','images', '*' + config['img_ext']))
    ...
    for epoch in range(config['epochs']):
        ...
        end_time = time.time()
        average_inference_time_ms = (end_time - start_time) / (len(val_loader.dataset)) * 1000

        print(f"Average Inference Time: {average_inference_time_ms:.3f} ms, length={len(val_loader.dataset)}")
